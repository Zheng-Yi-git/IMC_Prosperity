...
class Trade:
    ...
    def resin(state: Status) -> list[Order]:
        fair_price = 10000  # 固定锚定价格
        edge = 2            # 做市时挂单偏离
        threshold = 1       # 抢单阈值
        clear_width = 2     # 清仓允许的价格偏差
        order_amount = 20
        orders = []

        # === 1. 主动抢单逻辑（错价机会） ===
        if state.best_bid > fair_price + threshold:
            qty = min(state.possible_sell_amt, order_amount)
            if qty > 0:
                orders.append(Order(state.product, state.best_bid, -qty))
                return orders

        if state.best_ask < fair_price - threshold:
            qty = min(state.possible_buy_amt, order_amount)
            if qty > 0:
                orders.append(Order(state.product, state.best_ask, qty))
                return orders

        # === 2. 仓位极限时，尝试 0 EV 平仓 ===
        near_limit = abs(state.rt_position) >= 0.9 * state.position_limit

        if near_limit:
            if state.rt_position > 0:
                # 多仓，尝试卖出清仓
                target_price = round(fair_price + clear_width)
                for price, amt in sorted(state.bids, reverse=True):
                    if price >= target_price:
                        qty = min(abs(amt), state.rt_position, state.possible_sell_amt, order_amount)
                        if qty > 0:
                ...
                # 空仓，尝试买回清仓
                target_price = round(fair_price - clear_width)
                for price, amt in sorted(state.asks):
                    if price <= target_price:
                        qty = min(abs(amt), -state.rt_position, state.possible_buy_amt, order_amount)
                        if qty > 0:
                            orders.append(Order(state.product, price, qty))
                            return orders

        # === 3. 做市行为（以 fair ± edge 挂单） ===
        bid_price = fair_price - edge
        ask_price = fair_price + edge

        bid_qty = min(state.possible_buy_amt, order_amount)
        ask_qty = min(state.possible_sell_amt, order_amount)

        if bid_qty > 0:
            orders.append(Order(state.product, bid_price, bid_qty))
        if ask_qty > 0:
            orders.append(Order(state.product, ask_price, -ask_qty))
    ...
    def orchids(state: Status) -> list[Order]:
        ...
        orders.extend(
            Strategy.exchange_arb(
                state=state, fair_price=current_price, next_price_move=0
            )
        )
    ...
    @staticmethod
    def gift_basket(
        basket: Status, chocolate: Status, strawberries: Status, roses: Status
    ) -> list[Order]:

        orders = []
        orders.extend(
            Strategy.index_arb(basket, chocolate, strawberries, roses, threshold=30)
        )
