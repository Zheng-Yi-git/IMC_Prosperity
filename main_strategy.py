...
class Strategy:
    ...
    def index_arb1(
        ...
        if z_score < -z_score_threshold:
            possible_buy_amt = min(
                ...
                jam.best_bid_amount // 3,
                djembe.best_bid_amount,
                int((basket.position_limit - basket.rt_position)*target_pos),
            ...
            #possible_buy_amt = int(possible_buy_amt*target_pos)
            orders.append(
                Order(
                    basket.product,
                    int(real_best_ask),
            ...
            #possible_sell_amt = int(possible_sell_amt*target_pos)
            orders.append(
                Order(
                    basket.product,
                    int(real_best_bid),
    ...
    def index_arb2(
        ...
        real_best_bid = basket.best_bid
        real_best_ask = basket.best_ask
        etf_best_bid = 1.5 * basket2.best_bid + 1 * djembe.best_bid
        etf_best_ask = 1.5 * basket2.best_ask + 1 * djembe.best_ask
        buy_real_price_diff = max(etf_best_bid - real_best_ask, 0)
        sell_real_price_diff = max(real_best_bid - etf_best_ask, 0)
        cur_arb_pos = basket.rt_position
        orders = []
        ...
        if len(real_minus_etf_vwap) > window:
            real_minus_etf_vwap.pop(0)
        diff_std = np.std(real_minus_etf_vwap)
        diff_std = 95  # based on edgar's historical data
        # maybe add diff_std based on historical data since can be skewed
        z_score = cur_real_minus_etf_vwap_diff / (diff_std + 1e-8)
        ...
        if cur_arb_pos > 0 and cur_real_minus_etf_vwap_diff > -close_threshold:
            ...
            orders.append(
                Order(
                    basket2.product,
                    int(basket2.best_ask),
                    int(possible_sell_amt * 1.5),
                )
            )
            orders.append(
                Order(
            ...
            cur_arb_pos -= possible_sell_amt
            basket2.rt_position_update(
                basket2.rt_position + int(possible_sell_amt * 1.5)
            )
            djembe.rt_position_update(djembe.rt_position + int(possible_sell_amt))
        elif cur_arb_pos < 0 and cur_real_minus_etf_vwap_diff < close_threshold:
            possible_buy_amt = min(
                basket.best_ask_amount,
                basket2.best_bid_amount // 1.5,
                djembe.best_bid_amount,
                -cur_arb_pos,
            )
            orders.append(
            ...
            orders.append(
                Order(
                    basket2.product,
                    int(basket2.best_bid),
                    -int(possible_buy_amt * 1.5),
                )
            )
            orders.append(
                Order(
            ...
            cur_arb_pos += possible_buy_amt
            basket2.rt_position_update(
                basket2.rt_position - int(possible_buy_amt * 1.5)
            )
            djembe.rt_position_update(djembe.rt_position - int(possible_buy_amt))
        ...
        if z_score < -z_score_threshold:
            possible_buy_amt = min(
                ...
                (basket2.position_limit + basket2.rt_position) // 1.5,
                (djembe.position_limit + djembe.rt_position),
            )
            orders.append(
                Order(
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        round1_product = False
        if round1_product == True:
        ...
        if PB1==True:
            ...
            result["PICNIC_BASKET2"] = Trade.picnic_basket2(
                self.state_picnic_basket1,
                self.state_picnic_basket2,
                self.state_djembes,
                self.PB1_real_minus_etf_vwap,
            )
